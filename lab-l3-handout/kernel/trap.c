...
usertrap(void)
{
  ...
  if(r_scause() == 8){
    ...
    syscall();
        ...
        if (*pte & PTE_COW){
            // Should allocate a new page, which should be writable, and copy the data from the COW page.
            uint flags = PTE_FLAGS(*pte); // Get the flags from the page table entry.
            flags |= PTE_W; // Make it writable
            ...
            // Now one less page is referencing the old page. Should unmap it, and free the memory if there is none that references it.
            ...
            // It wasnt a COW page, but got this error anyway. Kill the process.
            p->killed = 1;
            panic("Something went wrong. ");
        }
